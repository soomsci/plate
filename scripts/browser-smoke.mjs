...
try {
  ...
  await waitFor(student, `document.querySelector('#map-tab').classList.contains('is-active') && !document.querySelector('#upload-earthquakes').disabled && !document.querySelector('#upload-volcanoes').disabled && [...document.querySelectorAll('[data-data-chapter]')].every((button) => button.disabled) && !document.querySelector('#data-empty-state').classList.contains('is-hidden') && [...document.querySelectorAll('[data-chapter-panel]')].every((panel) => panel.classList.contains('is-hidden')) && !globalThis.__plateStudentMap.map.hasLayer(globalThis.__plateStudentMap.layers.earthquakes) && !globalThis.__plateStudentMap.map.hasLayer(globalThis.__plateStudentMap.layers.volcanoes) && globalThis.__plateStudentMap.layers.countries.getLayers().length === 3 && globalThis.__plateStudentMap.layers.boundaries.getLayers().length === 3 && getComputedStyle(document.querySelector('.leaflet-draw-draw-polyline'),'::after').content.includes('선 그리기') && getComputedStyle(document.querySelector('.leaflet-draw-edit-edit'),'::after').content.includes('선 수정')`);
